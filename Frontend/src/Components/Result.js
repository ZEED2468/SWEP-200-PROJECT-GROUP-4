...
=======
  ...
  const student = location.state?.student || {}; // Default to empty object if student is undefined

  // Redirect if student.matricNo is not present
  if (!student.matricNo) {
    return <Navigate to="/notfound" />; // Redirect to an error page or any other page
  }

  // Ensure correct data display
  const currentPart = student.currentPart || 'N/A';
  const semester = student.semester || 'N/A';
  const image1 = student.image1 || 'default-profile.png'; // Use a default image if not available
>>>>>>> df49ae1a38c3fe329558e38352a161da525dbea9
